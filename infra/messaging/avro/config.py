...
from infra.messaging.avro.schema_registry import SchemaRegistryClient, CompatibilityLevel
        ...
        try:
            ...
            if topic_config.value_schema:
                value_id = await schema_registry_client.register_schema(
                    topic_config.value_subject,
                    topic_config.value_schema
                )
                print(f"Value 스키마 등록: {topic_config.value_subject} (ID: {value_id})")
            
            # Key 스키마 등록 (있는 경우)
            if topic_config.key_subject and topic_config.key_schema:
                key_id = await schema_registry_client.register_schema(
                    topic_config.key_subject,
                    topic_config.key_schema
                )
            ...
            await schema_registry_client.set_compatibility_level(
                topic_config.value_subject,
                topic_config.compatibility_level
            )
