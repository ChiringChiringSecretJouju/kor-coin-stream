...
class BaseKoreaWebsocketHandler(BaseWebsocketHandler):
    ...
    async def _initialize_batch_system(self) -> None:
        ...
        try:
            ...
            logger.info(
                ...
                f"(Korea optimized, shared_producer={self._is_shared_producer})"
            )
    ...
    async def trade_message(self, message: Any) -> TradeResponseData | None:
        ...
        if self._batch_enabled and self._batch_collector:
            ...
            logger.debug(
                f"{self.scope.exchange}: Trade message added to batch collector"
            )
        else:
            logger.warning(
                f"""
