...
from src.core.connection.utils.logging.log_phases import (
    PHASE_HEALTH_MONITOR_START,
    PHASE_HEALTH_MONITOR_STOP,
    PHASE_HEARTBEAT_LOOP,
    PHASE_HEARTBEAT_SEND,
    PHASE_WATCHDOG_IDLE,
    PHASE_WATCHDOG_LOOP,
)
...
class ConnectionHealthMonitor(ScopedConnectionLoggingMixin):
    """연결 상태 감시 전담 클래스
    ...
    def __init__(self, scope: ConnectionScopeDomain, policy: ConnectionPolicyDomain) -> None:
        self.scope = scope
        self.policy = policy
        self._logger = logger
    ...
    async def send_heartbeat(self, websocket: Any) -> None:
        ...
        try:
            ...
            self._log_debug("하트비트 전송 성공", phase=PHASE_HEARTBEAT_SEND)

        except websockets.exceptions.ConnectionClosed:
            # 연결이 닫힌 경우 경고 로그 없이 디버그 로그만 남기고 상위로 전파
            self._update_heartbeat_status(success=False)
            self._log_debug("하트비트 전송 중 연결 종료됨", phase=PHASE_HEARTBEAT_SEND)
            raise

        except Exception as e:
            self._update_heartbeat_status(success=False)
            self._log_warning("하트비트 전송 실패", phase=PHASE_HEARTBEAT_SEND, error=str(e))
            await self._emit_error(e, phase=PHASE_HEARTBEAT_SEND)
            raise
    ...
    async def start_monitoring(self, websocket: Any, ping_interval: int) -> None:
        ...
        # 하트비트 태스크 시작
        self._heartbeat_task = asyncio.create_task(self._heartbeat_loop(websocket, ping_interval))
        ...
        self._log_info("연결 상태 모니터링 시작", phase=PHASE_HEALTH_MONITOR_START)
    ...
    async def stop_monitoring(self) -> None:
        ...
        self._log_info("연결 상태 모니터링 중단", phase=PHASE_HEALTH_MONITOR_STOP)
    ...
    async def _heartbeat_loop(self, websocket: Any, interval: int) -> None:
        ...
        while self._is_monitoring:
            try:
                ...
                self._log_info(
                    "연결 종료로 인해 하트비트 루프 중단",
                    phase=PHASE_HEARTBEAT_LOOP,
                )
                break
            except Exception as e:
                self._log_error("하트비트 루프 에러", phase=PHASE_HEARTBEAT_LOOP, error=str(e))
                await self._emit_error(e, phase=PHASE_HEARTBEAT_LOOP)
                break
        ...
        while self._is_monitoring:
            try:
                ...
                if timeout > 0 and idle_for >= timeout:
                    self._log_warning(
                        "receive idle timeout exceeded",
                        phase=PHASE_WATCHDOG_IDLE,
                ...
                self._log_error("watchdog loop error", phase=PHASE_WATCHDOG_LOOP, error=str(e))
                await self._emit_error(e, phase=PHASE_WATCHDOG_LOOP)
                break
