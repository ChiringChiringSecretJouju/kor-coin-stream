...
def to_connection_scope(target: ConnectionTargetDTO) -> ConnectionScopeDomain:
    """ConnectionTargetDTO에서 ConnectionScopeDomain을 생성하는 공용 헬퍼."""
...
def adapter_stream_context(dto: CommandDTO) -> StreamContextDomain:
    ...
    if isinstance(dto, BaseModel) and not isinstance(dto, CommandDTO):
        raise TypeError("adapter_stream_context expects CommandDTO (validated Pydantic model)")

    # scope 구성
    scope: ConnectionScopeDomain = to_connection_scope(dto.target)
