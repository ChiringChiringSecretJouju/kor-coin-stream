...
class BatchMonitoringProducer(AvroProducer):
    """배치 성능 모니터링 이벤트 Producer (JSON 직렬화 기본).

    토픽: monitoring.batch.performance
    키: {region}:{exchange}:{request_type}
    스키마: batch-monitoring-value (선택적 Avro)
    ...
    def __init__(self) -> None:
        ...
        resolved_use_avro, _ = resolve_use_avro_for_topic(self.topic, False)
    ...
    async def send_summary_event(self, event: PerformanceSummaryEventDTO) -> bool:
        ...
        # event_timestamp_utc 추가
        payload["event_timestamp_utc"] = time.strftime("%Y-%m-%dT%H:%M:%S.%fZ", time.gmtime())
