...
def new_topic_initialization(
    topic: list[str], partition: list[int], replication: list[int]
) -> None:
    """new topic create

    Args:
        topic (list[str]): topicname
        partition (list[int]): kafka partition
        replication (list[int]): replication in kafak partition
    """
